...
class Bgaussian(object):
    ...
    def __init__(self, B, kH, kL, q, **kwargs):
        ...
        #print("Un=",self.__Un )
        #print("Un.shape=",self.__Un.shape )
        return
    ...
    def __init_k_array(self):
        ...
        #self.__Un = np.linspace(1/self.__kn.shape[0],1,self.__kn.shape[0])
        #self.__Vn = np.linspace(1/self.__kn.shape[0],1,self.__kn.shape[0])
        self.__Un = rand(self.__kn.shape[0])
        self.__Vn = rand(self.__kn.shape[0])
        return
    ...
    def new_random_numbers(self):
        ...
        ###self.__Un = rand(self.__kn.shape[0])####
        ###self.__Vn = rand(self.__kn.shape[0])###
        print("kn.shape[0]=", self.__kn.shape[0])
        return
    ...
    def __corr_transversal(self, k):
        ...
        """
        #B=np.linspace(1,100,4499)
        #spatial_corr = pi / 4. * B * B * self.Fq(k / self._kH)
        spatial_corr = pi / 4. * self._B * self._B * self.Fq(k / self._kH)

        #plt.plot(k/self._kH,self.Fq(k / self._kH))
        return spatial_corr
    ...
    def Bgaus(self, z, longitudinal=False):
        ...
        #self.__kn = np.linspace(self.kMin, self._kH, 4499)
        if longitudinal:
        ...
        #print("kn shape ", self.__kn.shape)
        #B = sum(sqrt(corr / pi * dd * 2. * log(1. / 0.5))
        #        * cos(3650.969543081127 * sum(zz)/len(zz) + 2. * pi * 0.5), axis=0)
        plt.semilogy(B,z)
        #print("zz avg=",sum(zz)/len(zz))
        ####################3
